...
public class FirstFit {
	...
	public static void main(String[] args) throws UnknownHostException, IOException {
		...
		while(msgReceived.substring(0,4).equals("JOBN")){			//check if server sent job 'JOBN'
			...
			if(args[1].substring(0, 2).equals("al") && args[0].substring(0, 2).equals("-a")) { 
				...
				while(!msgReceived.substring(0, 1).equals(endResc)) {
					...
					if(sState == 2 || sState == 3) {
						if(isServer_enough_resource(sCPU, sMem, sDisk, jobCPU, jobMem, jobDisk)) {
							System.out.println("server state = 23");
							msgSCHD(s, jobID, sType, sID);
							schdComplete = true;
							value = "23";
						} 
					} else if(sAvailTime < minAvail && !schdComplete) {
						System.out.println("server state = 14");
							minAvail = sAvailTime;
							msgSCHD(s, jobID, sType, sID);
							value = "14";
					} 
					...
					//if no servers have sufficient available resources to run the job, 
			...
			if(value.equals("23")) {count23++;}
			if(value.equals("14")) {count14++;}
			System.out.println("Count23: " + count23);
			System.out.println("Count14: " + count14);
			System.out.println("number of jobs: " + numJob);
